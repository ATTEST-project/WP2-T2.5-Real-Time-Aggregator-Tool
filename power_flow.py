...
def power_flow_elec(m, s, branch, load_in_bus, other_w):
    ...
    for t in range(h, h + 1):
        for i in range(0, len(branch)):
            ...
            if s == 0:
                ...
                m.c1.add(m.PF[scenario, i, t] == m.P_dso[scenario, branch[i][1].astype(int), t]/branch_div -
                         m.Pres[scenario, branch[i][1].astype(int), t] / 1 + somaP + branch[i][2] * (m.I[scenario, i, t]) / branch_div)
                ...
                m.c1.add(m.QF[scenario, i, t] == m.P_dso[scenario, branch[i][1].astype(int), t] * 0.2/branch_div -
                         m.Qres[scenario, branch[i][1].astype(int), t] / 1 + somaQ + branch[i][3] * (m.I[scenario, i, t]) / branch_div)
            ...
            m.c1.add(m.V[scenario, branch[i][0].astype(int), t] - 2 * (branch[i][2] * m.PF[scenario, i, t] + branch[i][3] * m.QF[scenario, i, t])/branch_div +
                     ((branch[i][2]/branch_div) ** 2 + (branch[i][3]/branch_div) ** 2) * m.I[scenario, i, t] == m.V[scenario, branch[i][1].astype(int), t])
        ...
        for j in range(0, len(load_in_bus)):
            if j != ref_bus:
                m.c1.add(m.V[scenario, j, t] >= v_min ** 2)
                m.c1.add(m.V[scenario, j, t] <= v_max ** 2)
